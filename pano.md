...
After you have the homography matrix, you need to calculate a bounding box for each image after applying the homography (where do the 4 corners of an image go after applying a homography?). Then you need to find the largest bounding box for all the images and use the dimension of the bounding box as your canvas size. After obtaining the canvas size, left multiply the homography matrix with a translation matrix to shift the upper-left corner to $$(0,0)$$ so that all the images can fit inside the canvas.
